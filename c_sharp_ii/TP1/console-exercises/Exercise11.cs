...
public class Exercise11 : IExercise
{
    ...
    {
        Console.WriteLine("=== Manipulação de Strings com Delegates Encadeados ===");
        Console.WriteLine();
        ...
        string concatenatedResult = concatenateNames(firstName, lastName);
        Console.WriteLine($"Após concatenação: '{concatenatedResult}'");

        string upperResult = toUpperCase(concatenatedResult);
        Console.WriteLine($"Após conversão para maiúsculas: '{upperResult}'");

        string finalResult = trimWhiteSpace(upperResult);
        ...
        Console.WriteLine("1. O Func<string, string, string> recebe dois parâmetros e retorna uma string");
        Console.WriteLine("2. Os métodos são executados sequencialmente, não em multicast");
        Console.WriteLine("3. Cada operação transforma o resultado da anterior");
        Console.WriteLine("4. O resultado final é a composição de todas as transformações");

        Console.WriteLine();
        Console.WriteLine("=== Demonstração com Multicast Delegate ===");
        
        Func<string, string> processText = null;
        processText += text => { Console.WriteLine($"  Processando: {text}"); return text.ToUpper(); };
        processText += text => { Console.WriteLine($"  Processando: {text}"); return text.Trim(); };

        Console.WriteLine("Executando delegate multicast:");
        string multicastResult = processText?.Invoke($"  {firstName} {lastName}  ") ?? "";
        Console.WriteLine($"Resultado do multicast: '{multicastResult}'");
        Console.WriteLine("Nota: Em delegates multicast com retorno, apenas o último resultado é mantido!");
    }
}